...
class MethodNode(object):
    ...
    def genbb(self, smalitree, methodidmap):
        lineno = 1
        ...
        while lineno < len(self.buf):
            line = self.buf[lineno]
            segs = line.split()
            
            if segs[0][0] == ":":
                label_lineno[line] = lineno
                    
                if segs[0].startswith(":pswitch_data"):
                    switch_label = line
                    switch_dests[switch_label] = []
                    lineno += 2
                    ...
                    while lineno < len(self.buf):
                        ...
                        if segs[0] == ".end":
                            break
                        if segs[0] in switch_dests[switch_label]:
                            lineno += 1
                            continue
                        switch_dests[switch_label].append(segs[0])
                        lineno += 1
                elif segs[0].startswith(":sswitch_data"):
                    switch_label = line
                    switch_dests[switch_label] = []
                    lineno += 2
                    ...
                    while lineno < len(self.buf):
                        ...
                        if segs[0] == ".end":
                            break
                        if segs[2] in switch_dests[switch_label]:
                            lineno += 1
                            continue
                        switch_dests[switch_label].append(segs[2])
                        lineno += 1
            lineno += 1
        ...
        while lineno < len(self.buf):
            ...
            if segs[0].startswith("goto"):
                destline = label_lineno[segs[1]]
                if lineno not in line_child.keys():
                    line_child[lineno] = []
                if destline not in line_parent.keys():
                    line_parent[destline] = []
                
                line_child[lineno].append(destline)
                line_parent[destline].append(lineno)
                
            elif segs[0].startswith("return"):
                destline = -1   
                if lineno not in line_child.keys():
                    line_child[lineno] = []
                if destline not in line_parent.keys():
            ...
            elif segs[0].startswith("packed-switch") or segs[0].startswith("sparse-switch"):
                destlines = []
                if lineno not in line_child.keys():
                    line_child[lineno] = []
                for destlabel in switch_dests[segs[2]]:
                    destline = label_lineno[destlabel]
                    if destline not in line_parent.keys():
                        line_parent[destline] = []
                    line_child[lineno].append(destline)
                    line_parent[destline].append(lineno)
                    
            elif segs[0].startswith("if-"):
                destline = label_lineno[segs[-1]]
                if lineno not in line_child.keys():
                    line_child[lineno] = []
                if destline not in line_parent.keys():
            ...
            elif segs[0].startswith("invoke-"):
                methodname = segs[-1]
                if methodname in methodidmap.keys():
                    internal_method.append(lineno)
                
            lineno += 1
        
        lineno = 0
        
        endblock = basicblock.BlockNode()
        endblock.startline = -1
        endblock.endline = -1
        
        currentblock = basicblock.BlockNode()
        currentblock.startline = 1
        
        while lineno < len(self.buf):
            line = self.buf[lineno]
            
            if lineno in line_parent.keys() or lineno in internal_method:
                currentblock.endline = lineno - 1
                self.blocks.append(currentblock)
                
                currentblock = basicblock.BlockNode()
                currentblock.startline = lineno
            
            if line[0] != ".": currentblock.insns.append(line)
            if lineno in line_child.keys():
                currentblock.endline = lineno
                self.blocks.append(currentblock)
                
                currentblock = basicblock.BlockNode()
                currentblock.startline = lineno + 1
            
            lineno += 1
        if (not currentblock in self.blocks) and len(currentblock.insns) != 0:
            currentblock.endline = lineno - 1
            self.blocks.append(currentblock)
        ...
        for b in self.blocks:
            line_block[b.startline] = b
            line_block[b.endline] = b
        
        for b in self.blocks:
            if b.startline in line_parent.keys():
                for linenum in line_parent[b.startline]:
                    b.parents.append(line_block[linenum])
            
            if b.endline in line_child.keys():
                for linenum in line_child[b.endline]:
                    b.children.append(line_block[linenum])
